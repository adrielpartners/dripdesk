...
Phase 9 implementation stores campaigns in `campaigns`. Campaign primary keys and all `campaign_id` foreign keys use six uppercase alphanumeric characters. IDs are globally unique, randomly generated by PostgreSQL, and shown unchanged in URLs, API responses, reports, and webhook requests. Incoming campaign IDs are case-insensitive; the canonical stored and returned form is uppercase. Existing UUID campaign IDs are replaced by the migration, so old campaign URLs and sender configurations must be updated. Other entity IDs remain UUIDs. All campaign reads and writes are scoped by `organization_id` through tenant-aware repositories. Campaign activation is allowed only when the campaign has required metadata, at least one default delivery channel, and at least one non-archived step with deliverable content.
